;(function(){
	...
	$(".path-step").on("click",(ev)=>{
		...
		const posision = $current_circle.index(".path-step") + 1

		let $test = $(".step:nth-child("+posision+")")
		siguiente($test)
	})
	...
	$(selector).find(".input").on("change",(ev)=>{
		...
		const is_valid = es_valido_formulario()

		if (!is_valid && $next_step.length > 0) {
			siguiente($next_step)
		} else {
			validar_formulario()
		}
	...
	function enviar_formulario(){
		...
		$.ajax({
		    ...
		    success: function(){
		    	$form.slideUp()
		    	$("#info-contacto").html("Tu mensaje a sido enviado")
		    }
		}); 
	}
})()